...
fig, (axP, axQ) = plt.subplots(nrows=2, ncols=1, constrained_layout=True)

axP.plot(n, N_1YSinF, 'lightgrey', label='1 $Y/Y^{S}$ chromosome')
axP.plot(n, N_2YSinF, 'grey', label='2 $Y/Y^{S}$ chromosomes')
axP.legend(loc='upper center')
axP.set_xlim(0, generations)
axP.set_ylim(0, population/2)
axP.set_ylabel("# of females")

axQ.plot(n, N_1YSinM, 'lightgrey', label='1 $Y/Y^{S}$ chromosome')
axQ.plot(n, N_2YSinM, 'grey', label='2 $Y/Y^{S}$ chromosomes')
axQ.plot(n, N_3YSinM, 'black', label='3 $Y/Y^{S}$ chromosomes')
